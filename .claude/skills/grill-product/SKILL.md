...
- **Capture** — a settled, buildable position goes to `/capture-task`, ready to seed a later engineering session. One coherent ask is ONE parent task file at PRD altitude, even when it spans several surfaces or decision areas: group the requirements by area inside the single file and write no slices — decomposing into vertical slices is `grill-engineer`'s job at scoping, where its spec-it exit owns Design decisions, Test strategy, and Slices against the real code, context this session does not have. Multiple task files are reserved for genuinely unrelated asks the user brought as separate items; related areas that crystallised out of one product position never qualify. If a split still seems warranted, put parent-vs-split to the user as its own explicit question with the single parent as the recommendation — a granularity choice pre-bundled inside one accept/reject confirm option (e.g. "write five scoped tasks?") is a decision the user never actually got to make.
