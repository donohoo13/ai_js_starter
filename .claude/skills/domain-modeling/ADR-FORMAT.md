...
> [!WARNING]
> This is the skill's own discipline to hold, checked on every ADR write: no ADR ships without a `status`; a `status: superseded` or `amended` names its successor; and a `supersedes`/`amends` claim only lands over an ADR that acknowledges it back. The back-link fields (`superseded-by`, `amended-by`) claim nothing about another file, so they are written freely, and that fixes the order in every case, new corpus or old: annotate the older ADR first, then write the newer one against an acknowledgement that already exists. A `guard-adr-links` PreToolUse hook enforced this until v1.0.7, when it was removed for only ever firing on AI-authored writes — a human editing an ADR in an editor bypassed it entirely, so it taxed one author and enforced nothing. A repo that wants this checked mechanically runs it as a lint over every ADR regardless of who wrote it, which is where a content check belongs.
