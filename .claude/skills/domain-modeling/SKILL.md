...
- **Shape change** — work in this session added a module, moved a boundary, changed a data flow, or rewired who-calls-whom. Update the owning context's doc in the same pass, and the root doc if cross-context topology moved. During a pure design conversation, hold off: the doc records shape only once it is real — the intent and its reasoning belong in an ADR, and the shape update lands with the code that lands it.
