---
name: capture-task
description: Quick-capture any unit of work (a bug, a feature idea, or a chore) as a structured task file in `docs/tasks/`. A "task" is the umbrella: anything worth tracking for later without running a full grilling session right now. Use whenever the user says "capture this", "log this", "track this for later", "file a follow-up", "create a task", "outline a feature", "we should do X later", "add this to the roadmap", "tech debt note", "park this", OR reports something broken to park for later: "file an issue for this", "demo bug", "saw a bug in prod", "this just broke". Investigating a bug now is `/diagnose`; parking it for later is this skill. A sweep of many features arriving at once to organize into projects is `grill-initiative`'s trigger, not a batch of captures. Optimized for low-friction in-the-moment capture.
...
Start from `assets/task-template.md` (sibling of this SKILL.md). It defines the required frontmatter and seven sections: Context, Problem, Scope, Requirements, Acceptance criteria, Dependencies, Risks / open questions. Replace every guidance comment with real content or `TBD (needs grilling)`; no `<!-- -->` comments survive in the captured file. Keep every bullet one continuous line. The template also defines the status lifecycle, the `incumbent:` frontmatter key, and the scoped-work sections (Design decisions, Test strategy, Slices); those belong to later stages — `grill-engineer` flips `captured → scoped` and is the only writer of `incumbent:`, `implement-task` takes it to `done` — so at capture, omit the scoped sections and the `incumbent:` key entirely and leave `status: captured`. A guessed verdict here is worse than an absent one: the absence is a gate `implement-task` enforces, while a guess satisfies that gate without anyone having made the call.
