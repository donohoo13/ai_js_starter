...
2. **Read every seat's Actions section before its findings**, and read it as evidence rather than as reassurance. A command with its output is checkable: run your eye down the numbers and see whether each conclusion actually follows, because a seat can run the right command and still draw the wrong inference from it. An attempt narrative is judgeable a different way: ask whether that was the right thing to try. What you must never do is convert either into a coverage claim of your own — a paraphrase is a fresh verdict about absence, and the report carries the seat's words for exactly that reason.
