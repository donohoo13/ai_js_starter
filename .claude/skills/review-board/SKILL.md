---
name: review-board
description: Multi-agent review board. Spawns parallel specialist reviewers — five code seats (correctness, security, reliability, maintainability, performance/operations) or three documentation seats (flow continuity, coherence, adversarial), chosen by what the change actually touches — consolidates their findings, renders the session AI's own confirmed/plausible/rejected verdict on each, and waits for the human to pick what gets addressed before anything is edited. Use whenever the user asks to review code changes, a branch, a diff, or a PR; asks for a security review, standards check, or pre-merge/pre-PR review; says "review my changes", "run the review board", or "is this safe to merge"; wants a second opinion on work in progress, even if they name only one concern; or asks to review a documentation, process, runbook, prompt, or agent-instruction change, where the document is the behavior and nothing compiles it. An optional leading mode argument (`quality`|`balanced`|`speed`) scales the board; with no mode given, the session AI opens by recommending a mode and read depth reasoned from the change itself.
...
**Diff-first is the default read depth below `quality`; full-file is an escalation a seat earns through a named trigger.** Diff-first means working from the diff and opening surrounding code only to confirm a suspected finding, so its recall loss is bounded; full changed-file reads are what multiply the board's cost. Escalate a seat only when one of these fires:
...
With no keyword, do not silently assume anything. Open with two recommendations, each carrying reasoning specific to this change:
...
2. **Depth posture** — diff-first for every seat unless a trigger above fires; name each escalated seat and its trigger ("I touched the auth middleware, so security reads full files; everything else diff-first").
...
Three seats, never consolidated below three even on a small change: they hunt by genuinely different methods — traversal, agreement, and exploitation — and each reliably finds what the others cannot. Adversarial is the deepest-reasoning seat and empirically the highest-yield, so it never drops below Sonnet; coherence is the most systematic, grep-shaped work and covers from Haiku up.

**Read depth inverts on this board: full changed-file reads are the default and diff-first is the exception.** A prose diff loses the section that gives it meaning, and the defect frequently sits in the untouched sentence beside the edit. The code seats' escalation triggers still apply for reading further into unchanged neighbors, plus one of this board's own: a corpus that cross-references heavily, where coherence earns the wider sweep.

A mixed change seats from both sets under the rule seat-skipping already uses — run a seat when its category has a surface in the change. A feature that ships code plus the runbook explaining it needs both halves; a pure prose fix in a README needs neither board and a plain read.
...
- **Read depth finalization per running reviewer (`balanced` only — the other modes preset it).** Apply the default-and-triggers rule above: diff-first for every seat, full files only where a named trigger fires, decided per seat rather than board-wide. If you authored the change, you already know which triggers fire; a gate-1 depth posture carries straight into this list rather than being re-litigated.
