...
- Seat security as a fourth — four seats rather than three, taking its row from the code matrix below — on either of two triggers, because a document reaches security by instructing a reader or by being the enforcement itself:
  ...
  - **The document controls**: the changed artifact is a control surface — permission and settings config, hooks, CI workflows, agent tool grants. These rarely tell anyone anything, so an instruction-shaped trigger never fires on the one class where a defect is a live hole rather than bad advice; a permission entry with no mirrored spelling, or a hook whose prefilter misses the pattern it guards, reads as clean prose to every other seat. This trigger reads the artifact's class and not the diff's size, because config has no trivial diff: a comment can disable a step and an indentation change can reparent a key, so the change that looks inert on the way past is exactly the one the seat is here for.
...
- Commit the fixes on a non-default branch, staged by explicit path, each message naming the finding it closes. These are the only commits a board produces, because a commit records a change to the code and nothing else about the review is one.
...
- **The review stays on this machine.** The full report file lives in the gitignored `.ai/` namespace and nothing about the board — verdicts, Actions, dismissal reasons — enters a commit message, a PR body, or any other surface that reaches the remote, unless the user asks for it there. A branch carries the work, not the deliberation about the work.
- That locality has a real cost, accepted deliberately: `.ai/` is worktree-local, so a fresh clone or a new worktree has no board record and none can be recovered. Re-running the board is the recovery. Restoring durability means an audit trail on the remote, which is a compliance posture rather than a template default — `project-init/references/fork-points.md` carries the add-back for regimes that need one.
