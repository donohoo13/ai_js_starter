...
- **Own the _how_; hand up the _what_:** this lens settles how to build, not what to build. If a genuine product-altitude question surfaces mid-grill — not "which component" but "what should this become for the user, and why", with no decided answer — that is `grill-product`'s ground: say so, route it there first, and resume the build here once the design position lands. Most UI work is not this — matching existing tokens, wiring states, and copy ride inside the build via `frontend-design`; reserve the hand-up for real design questions, not styling.
