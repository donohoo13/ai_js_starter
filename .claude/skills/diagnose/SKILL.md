...
10. **HITL script**, last resort: if a human must click, copy `scripts/hitl-loop.template.sh` out of the skill directory into the gitignored scratch path, edit the copy, and hand it to them to run in their own terminal, pasting the captured block back. Edit the copy, never the template in place — `guard-skill-edit` denies `Edit` and `Write` there while a Bash write is outside its matcher, so the instruction is the whole mechanism for that route — and a bug's steps written into the shipped file would ride out in the next release. Running it through the Bash tool reaches no human and returns an empty capture that reads like a real one.

Loops that need the app running (an HTTP script against the dev server, a headless browser driving the UI) drive a server the user starts: servers are user-run (CLAUDE.md), so hand the exact command and wait rather than launching it — the session is interactive, so the ask costs one line.
