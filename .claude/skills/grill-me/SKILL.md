...
The rule generalizes past the table: a design artifact routes toward its build, a `done` task gets "this is finished — did you mean to reopen it?", a `completed` initiative reports there is nothing left to pick up. A session picking up a portfolio project also carries the bookkeeping the flow expects: flip the row `queued → in progress` and link the brief when it lands, and write the brief's `initiative:` frontmatter — the row's `done` flip stays manual, on the user's judgment against its done-when.
