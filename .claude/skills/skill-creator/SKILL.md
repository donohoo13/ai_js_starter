...
- **Load-time snapshots run under the session's restrictions, not the author's.** Both snapshot forms — the inline bang-backtick and the bang-fenced block — execute before the body is available, in whatever context invoked the skill: a git worktree, a narrowed permission set, a sandbox. A refused snapshot aborts the load, so the skill's content never reaches the session at all and none of its rules run. Write the plainest command that yields the fact, let the body interpret the output, and state what empty output means beside it rather than encoding a fallback in shell — a snapshot of `git remote` labeled "empty = no remote" carries exactly what `[ -n "$(git remote)" ] && git remote | head -1 || echo "(none)"` carries and survives contexts the compound form does not. Command substitution is the shape that fails first, because a harness isolating the session cannot statically prove a substituted subcommand stays inside the boundary, so it refuses the whole command rather than guess. Reproduce the bang syntax itself nowhere outside a real snapshot, since an example of it in prose or a code span risks executing as one.
