...
2. **Capture carve-outs before anything is deleted.** A surface the task explicitly keeps exists only until demolition runs, so screenshot it now with the project's UI verification tool (named in `CLAUDE.md`), cropped to that surface and nothing else, and store it through `grill-design`'s existing path: `docs/assets/<artifact-slug>/` with per-breakpoint labeling, classified before storage per the data-handling rules, recorded in the design artifact's `source:` frontmatter. The dispatching session does this rather than the agent because it already has the UI tool wired, and driving a running app yields pixels rather than source, so the firewall holds. **Which surface survives is a design decision that arrives with the task** — a session choosing carve-outs here is authoring composition at build time with no design authority.
3. **Write the mandate.** The agent cannot see the conversation, so its prompt carries: the demolition zone in the task's own vocabulary, the carve-out list, the record's absolute path, and every design or brief statement that overrules an incumbent contract, quoted with its location.
...
**Announce the manifest, then proceed.** State what dies, what survives and why, what is suspected-dead-unverified, and what the archaeology turned up — then continue unless interrupted. This is the window the deep plan already uses, not an approval gate: a confirm on every file trades the speed that makes demolition worth doing for a signature nobody reads.
