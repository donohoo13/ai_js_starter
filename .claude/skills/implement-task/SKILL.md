...
- Read the `design:` artifact (or the governed-verdict line and its intent assertions when the task carries no artifact), `.claude/rules/ux-standards.md`, `BRANDING.md`, and the app's theme CSS before planning; add `.claude/rules/transactional-email.md` when the surface is an email.
...
- Judge a governed-verdict line against the component or design artifact it names and against `ux-standards.md`'s floors the same way; its recorded intent assertions are the feel contract, judged at the land render pass rather than statically.
...
- Judge against the artifact, Experience intent and Source fidelity included, and against `BRANDING.md` and `ux-standards.md`'s floors; fix what fails. On a governed-verdict task there is no artifact, so the intent assertions recorded in Design decisions are what the render is judged to feel like — read them from the task file, because nothing else on that path states feel.
