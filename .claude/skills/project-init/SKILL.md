...
- **Secret-file classification**: for each uncovered file from the secret sweep, ask secret or readable — one agenda item, not a file-by-file slog. Confirmed secrets become a plan item appending paired `Read(...)` and `Edit(...)` deny entries plus matching `guard-secret-read` prefilter tokens to `.claude/settings.json`, edited together so the hook's cheap pre-check never drifts from the registry it gates — `Read` deny alone leaves `Write` and `NotebookEdit` open, so an unmirrored entry protects nothing against a write; a dot-spelled example file (`.dev.vars.example`) gets renamed to the hyphen spelling rather than a loosened wildcard.
...
- **Repo shape**: the template ships a pnpm + Turborepo monorepo skeleton (`apps/*`, `packages/*`, `turbo.json`, workspace globs in `pnpm-workspace.yaml`); a single-app destination gets offered the collapse — delete the skeleton dirs and `turbo.json`, drop the workspace globs, de-turbo the `package.json` scripts.
- **License**: confirm the shipped `LICENSE` against the project's reality.
- **Node pin**: confirm it as a set rather than a file — retargeting means changing `.nvmrc` (exact version), `engines.node` (`>=X.Y.Z <X+1`), and `devEngines.runtime.version` (the same exact version, never a range) together, then running `pnpm install` and committing the regenerated `pnpm-lock.yaml`; `scripts/setup/check-install.mjs` hard-fails on drift between the first three, and its header comment carries the full reasoning for each choice, which is the only copy that survives this skill's self-removal.
- **Typechecker**: detection already reads the stack's typecheck command; its _absence_ is an agenda item rather than a recorded fact, because `implement-task`'s demolition pass reads the compiler's error set as the build's connection map and a project without one falls back to a materially weaker reference audit. State the cost and branch on it: wiring `pyright` into a `uv` project is a config file and a script, so offer that inline, while adopting TypeScript across a large JS app is a migration that would swallow this session, so hand it out as a captured task instead of starting it here.
...
- **Template residue and identity**: stamp `Template lineage: <owner>/<repo>, vX.Y.Z` into `CLAUDE.md` from the top `CHANGELOG.md` entry, then delete the local `CHANGELOG.md`.
  - Clear the template's own working artifacts (`docs/tasks/`, `docs/adr/`, `docs/designs/`, `docs/briefs/`, `docs/initiatives/`, `docs/assets/`, `docs/notes/`, `CONTEXT.md`, `ARCHITECTURE.md`, `CONTEXT-MAP.md`) and `.claude/rules/template-dev.md`; `docs/company/` stays, it is payload skeleton.
  - Rewrite `README.md` into the project's own, carrying the template-updates recipe forward; rename the `package.json` `name` field.
  - `sync-template` and `template-feedback` always survive — they are the update and feedback channels between instance and template, not init tooling, and both look like template machinery precisely because they are the two skills whose job is talking to the parent.
  - This residue set is mirrored by `sync-template`'s Phase 1 diff exclusion, which keeps later releases from offering the same files back; the two move together, so a path added here belongs there in the same change, and `.claude/skills/project-init/` itself is on that list because Phase 5 deletes it.
- **Agents**: verify every `review-*` agent's declared tools work here (LSP plugin + binary per doctor; the three documentation seats read prose and never need it), verify `research-analyst`'s Context7 and web dependencies, confirm `demolition-executor` can actually delete and `demolition-planner` can still write its record under this project's `permissions` (they are the only agents that touch the tree, so a narrowed `Bash`/`Write` allow disables the demolition pass silently), and apply any approved signal-gated checklist additions in `review-board/references/`.
