...
- **Template residue and identity**: stamp `Template lineage: <owner>/<repo>, vX.Y.Z` into `CLAUDE.md` from the top `CHANGELOG.md` entry, then delete the local `CHANGELOG.md`; clear the template's own working artifacts (`docs/tasks/`, `docs/adr/`, `docs/designs/`, `docs/briefs/`, `CONTEXT.md`, `ARCHITECTURE.md`, `CONTEXT-MAP.md`) and `.claude/rules/template-dev.md` — `docs/company/` stays, it is payload skeleton; rewrite `README.md` into the project's own, carrying the template-updates recipe forward; rename the `package.json` `name` field. `sync-template` always survives — it is the update channel, not init tooling.
