...
- `ship-pr/SKILL.md` + `.github/PULL_REQUEST_TEMPLATE.md` — the shipped PR body is descriptive only: Summary plus QA, no review reporting. → under audit requirements, add the reporting back at both ends:
  - Add a Review board section to the template and its semantics to the skill: mode and date, findings by verdict, addressed IDs with fix commit SHAs, dismissed IDs each with the user's recorded one-line reason — where none was recorded, write `no reason recorded`, never a reconstructed one — and every seat's Actions evidence.
  - Re-couple by reading `review-board`'s record commits — `git log <default>..HEAD --grep='^review:'` — with the latest record as the final state.
  - Carry Actions into the PR body verbatim, because a squash merge destroys the record commit and the PR body is the only audit surface that survives; a board reported as "0 findings" without its seats' commands and outputs is indistinguishable from a board that never looked.
