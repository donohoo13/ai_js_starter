...
- `grill-design/SKILL.md` — designs live at `docs/designs/`, task files back-link via `design:`, and sources are stored under `docs/assets/<artifact-slug>/` and named in `source:` frontmatter, which is what triggers the source-versus-render comparison. That directory holds a second resident the `source:` key never names: the compiled external-tool prompt a mockup round trip stores before handing over, which the returning session picks up as a step of the fidelity contract to check the mockup against the position it was compiled from. It is also the one resident that leaves the machine, since the same text goes to a third-party design tool, so a regime that retargets this directory weighs the external tool alongside the storage path rather than only the storage path. → when designs live outside the repo:
...
- `implement-task/SKILL.md` Section 2 Step 3 — the doc pre-flight inlines `stage-for-commit`'s behavior rather than invoking it, so the entry above never reaches it: the offer ends with the user committing on the default branch, which is correct wherever a human can commit there and unexecutable where an org ruleset routes every change through a pull request. → under a PR-required regime, move the destination and keep every branch switch the user's. The session derives the task branch per Section 2 Step 1 and hands over the commands; the user creates that branch in the main checkout so the untracked docs ride along, commits there, and returns the checkout to the default branch before Section 2 Step 4 runs. Keeping the actor matters as much as moving the destination: Section 2's second bullet forbids the session switching branches in a shared checkout, so a tailored Step 3 that has the session do it contradicts the section it sits in. The ordering is load-bearing and unguessable — `gwt-add.sh` attaches an existing branch with `git worktree add <target> <branch>`, git refuses while the main checkout still holds that branch, and Section 2 Step 4's non-zero stop is what fires when the return is skipped. Tailor the Fallback section alongside it, since a declined confirm then builds on the branch Step 3 already created rather than asking the user for one.
...
- `implement-task` Section 2 Steps 2 and 4 — reuses or creates the worktree via `scripts/setup/gwt-add.sh --no-open` plus `EnterWorktree`, and Step 3's doc pre-flight watches `docs/**` plus the context files. → a project dropping the scripts reverts to the ask-for-a-branch escape hatch; retarget the pre-flight path list when docs live elsewhere.
...
- `.claude/rules/ux-standards.md` (shipped) — 55 sourced usability and accessibility floors, `paths:`-scoped to component, markup, and style extensions. Ships identical to every project and is closed to growth, so init never edits its content. Its globs carry the same trap the JS/TS entry names: `.css` and `.html` match files in almost any repo, so the file activates in a destination with no UI unless it is deleted. → leave the content alone; delete the file and its `CLAUDE.md` pointer together when the destination ships no UI **and** sends no templated mail, because `transactional-email.md` defers its contrast, touch-target, and accessibility floors to this file by reference and states none of those numbers itself. A UI-less service that sends verification mail keeps both. A floor an instance believes is missing is a `/template-feedback` finding, never a local append, because the file's whole value is that it does not accumulate.
- `.claude/rules/frontend-styling.md` (shipped) — units, CSS-first, and a Tailwind v4 block, `paths:`-scoped to the same extensions. This is the one of the three that tracks the stack. → prune the Tailwind block and write the destination's real styling system in its place (CSS Modules, vanilla-extract, styled-components, Panda); delete the file and its pointer together when the destination ships no UI. The block's theme-values-by-name rule is not Tailwind-specific — a semantic colour token for every colour, and a custom property for every non-colour value that differs between themes, each declared once with a value under each theme selector — so carry it into the replacement section in that system's own idiom rather than pruning it away with the syntax that happened to express it.
- `.claude/rules/transactional-email.md` (shipped) — twelve email-client rules, `paths:`-scoped to email template locations (`**/email{s,}/**`, `**/mail/**`, `**/templates/**`, `*.email.*`). Most projects send no templated mail. → delete the file and its pointer together unless the destination sends transactional email; retarget the globs when its templates live somewhere the shipped set does not reach.
- The three rules files above and their `CLAUDE.md` pointer move together in every direction. A pointer left behind explains rules that no longer load, and a file left behind loads rules nothing explains — the same coupling the JS/TS entry states, and the reason both are named in one plan item rather than two.
- Template extensions outside the shipped globs (`.templ`, `.erb`, `.heex`, `.slim`) are the one case where init edits `paths:` rather than content, alongside a monorepo scoping a rules file to a single app's path. Adding a glob for an extension the destination genuinely uses is cheap; a glob that matches nothing costs nothing, so breadth is rarely the thing to tune — the exception is a pattern broad enough to catch the wrong files, which is why the email globs are scoped to mail-named directories rather than a bare `**/templates/**` that would reach every Django, Flask, and Express view directory.
...
- `demolition-planner.md` + `demolition-executor.md` — two dispatches from `implement-task`'s demolition dispatch section: the planner reads and records, and the executor holds `Bash` alone and deletes via `git rm` without `-f`. → verify tool viability, then:
...
- `review-board/SKILL.md` — **seat coverage**: seat-skipping allows dropping the security seat when a code change has no security surface, and on the documentation board security sits as a fourth only on its two triggers (the document instructs a reader about credentials, customer data, or outbound transfer, or the document is itself a control surface). → under a compliance regime, make the security seat non-skippable on the code board and drop the documentation board's triggers so it sits on every prose change, since a runbook telling an operator to paste a credential into a chat window is a real finding no other doc seat owns.
- `review-board/SKILL.md` — **durability**: the review stays on the machine. Fixes are committed on the branch, the outcome per finding is appended to the gitignored `.ai/review/` report, and nothing about the board reaches the remote, so a fresh clone holds no evidence a board ever ran. → where the regime needs an audit trail, restore a record commit as the skill's closing step:
  - Always empty and always last on a non-default branch, written even when the selection was `none`, so there is one shape to parse and it pins the record to the reviewed tree. Skip it on the default branch.
  - Subject `review: board (<mode>) — <N> findings, <X> addressed, <Y> dismissed, <Z> rejected`, which is a stated point-of-use exception to the project's 50-char commit rule; the `review:` prefix is the grep contract. Body: one line per finding (ID, severity, verdict, title, and the outcome — fix SHA, task path, the user's verbatim reason, or rejected), then every seat's Actions verbatim, then the trailers `Review-Mode:` and `Review-Scope: <base>..<head>`.
  - Write it with a quoted heredoc (`git commit --allow-empty -F - <<'RECORD'`), never `-m`: Actions contains backticks and both failure modes mimic success.
  - Strike what the restored commit contradicts, or the tailored skill mandates and forbids the same write: the close-out step's locality absolute (the board's deliberation reaches no outbound surface on its own) and Step 8's "these are the only commits a board produces" both go, restated around the record commit as the one exception.
  - Scrub before anything crosses, across the whole commit body and not just one field. Every literal credential, token, and PII value is replaced with its match count and `file:line` — in a seat's Actions, in a finding's title, and in the user's recorded decline reason, which is where such a value most naturally arrives, since a user explaining that a flagged match is harmless tends to name it. The raw line stays in the `.ai/review/` report. A scrubbed reason is still the user's words and still never a reconstructed one. A security seat's secret grep can hit a real value in a path no deny rule covers, and this commit is pushed with the branch — the evidentiary property is the command and its output, which a count and a location preserve.
  - Warn that an interactive rebase silently drops an empty commit, and that survival past merge depends on the merge method: a squash destroys the record, while a rebase or a merge commit carries it onto the default branch under a new SHA. A regime that cannot pin the merge method needs the PR-body copy below as well.
- `ship-pr/SKILL.md` + `.github/PULL_REQUEST_TEMPLATE.md` — the shipped PR body is descriptive only: Summary plus QA, no review reporting. → under audit requirements, add the reporting back on top of the restored record commit above:
  - Add a Review board section to the template and its semantics to the skill: mode and date, findings by verdict, addressed IDs with fix commit SHAs, dismissed IDs each with the user's recorded one-line reason — where none was recorded, write `no reason recorded`, never a reconstructed one — and every seat's Actions evidence, scrubbed the same way.
  - Re-couple by reading the restored record commits — `git log <default>..HEAD --grep='^review:'` — with the latest record as the final state.
  - Carry the record commit's already-scrubbed content into the PR body wherever the merge method can squash the record commit away, since the PR body is then the only audit surface that survives; a board reported as "0 findings" without its seats' commands and outputs is indistinguishable from a board that never looked. Copy from the scrubbed record rather than from the report file, so the filter is applied once and cannot be skipped at the second crossing.
  - A missing record reads as no board run and is written as `Not run.`, never omitted; write `Declined.` only when the decline happened where the session can see it, because a declined board leaves no record commit for a later session to find.
  - Where the regime demands enforcement rather than disclosure, a missing board is a hard stop: refuse the PR and route to `/review-board` instead of writing `Not run.` — reporting is the floor every audit fork adds, the hard stop is the stricter regime's escalation on top of it.
...
- `BRANDING.md` skeletal — read at runtime by the grill lenses, which reach it by name, and `brand-init` is the sole path for filling the skeleton. → report a pointer to `/brand-init` and retarget the mood-board directory if docs live elsewhere; no init-time edits to this file. The frontend rules files are a separate entry under Stack and toolchain, because two of the three are genuinely init-tailored.
