...
- `review-board/SKILL.md` — **durability**: the review stays on the machine. Fixes are committed on the branch, the outcome per finding is appended to the gitignored `.ai/review/` report, and nothing about the board reaches the remote, so a fresh clone holds no evidence a board ever ran. → where the regime needs an audit trail, restore a record commit as the skill's closing step:
  ...
  - Scrub before anything crosses. A literal credential, token, or PII value in a seat's Actions is replaced with its match count and `file:line`; the raw line stays in the `.ai/review/` report. A security seat's secret grep can hit a real value in a path no deny rule covers, and this commit is pushed with the branch — the evidentiary property is the command and its output, which a count and a location preserve.
...
- `ship-pr/SKILL.md` + `.github/PULL_REQUEST_TEMPLATE.md` — the shipped PR body is descriptive only: Summary plus QA, no review reporting. → under audit requirements, add the reporting back on top of the restored record commit above:
  ...
  - Carry Actions into the PR body verbatim wherever the merge method can squash the record commit away, since the PR body is then the only audit surface that survives; a board reported as "0 findings" without its seats' commands and outputs is indistinguishable from a board that never looked.
