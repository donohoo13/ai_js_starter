...
`BRANDING.md` voice rules and `CLAUDE.md`'s no-em-dash rule apply to all email copy, and the accessibility floors in [`ux-standards.md`](./ux-standards.md) apply to the rendered message. A project that sends no templated mail deletes this file; `project-init` offers that at onboarding.
