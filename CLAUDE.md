...
- `scripts/worktree.sh <add|remove|path|list|shim>` — worktree create (`add [--no-open] <branch>`: env copy, `pnpm install`, Zed unless `--no-open`), remove (worktree + branch + empty-parent prune), and navigation (`path [branch]` prints one path on stdout with the picker on stderr; `list` shows them). Worktrees live at `${WORKTREE_ROOT:-$HOME/.git-worktrees}/<project>/<branch>`. It resolves the main checkout itself, so it runs correctly from inside a worktree or any subdirectory.
- `scripts/doctor.sh [--fix [--dry-run]]` — bare: warn-only machine-prerequisite check (LSP server binaries), auto-run by `prepare`, always exits 0. `--fix`: user-invoked only, installs the missing binaries and appends the `wtree` shell function to your rc file; `--dry-run` shows the changes without writing.
...
- `pnpm test` (`scripts/test.sh`) — behavioral tests for the shipped shell scripts, run against a scratch `HOME` and throwaway git repos. Deliberately not wired into `prepare`: it creates repos and runs `pnpm install`. It is the only test step; the repo contains no application code, so there is no build or lint step.
- The pre-commit hook (`.husky/pre-commit`) auto-formats staged files with Prettier and re-stages them.
...
- `scripts/worktree.sh` (byte-identical twin in `src/scripts/`) is the git worktree helper behind implement-task's step-2 checkpoint: `add` creates the worktree at `${WORKTREE_ROOT:-$HOME/.git-worktrees}/<project>/<branch>` with env copy and `pnpm install`, `remove` tears it down, `path`/`list` navigate, `shim` emits the `wtree` function. The root is an env var, not a constant, because a hardcoded path is one machine's layout imposed on every project that adopts the template.
- This repo overrides Claude Code's native worktree support deliberately: `EnterWorktree` creates worktrees inside `.claude/worktrees/`, which is in-repo, is not configurable by any setting, and is not auto-gitignored. The `WorktreeCreate` hook that could redirect it fires only at session launch (`claude --worktree`, subagent isolation), so adopting it would force the worktree decision before the grilling that determines whether a worktree is warranted at all. Script and native tool compose instead: the script owns machine setup, `EnterWorktree path:` owns session relocation.
- `scripts/test.sh` (also twinned) is the behavioral suite for both scripts: scratch-`HOME` tests covering worktree lifecycle, rc-file mutation, the `wtree` shim's trust boundary, and bash-3.2/twin-drift assertions. `worktree.sh` and `doctor.sh` write to a user's home directory and delete directories, which is why they carry tests when nothing else here does.
- `scripts/doctor.sh` (also twinned) is the machine-prerequisite check: warn-only and wired into `package.json` `prepare` so every install self-reports gaps, with `--fix` as the user-invoked remediation that installs LSP binaries and the `wtree` shim. Bare and `--fix` are opposite contracts over one prerequisite list, kept in one file so a warning can never describe a fix nothing performs.
