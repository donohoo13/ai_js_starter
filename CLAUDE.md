...
- Corepack ships bundled through Node 24 and is removed from Node 25+. While the Node pin stays on 24, activate pnpm through Corepack and the `packageManager` field (see the README quickstart). Retargeting the Node major to 25 or newer is the trigger to migrate the pnpm activation path off Corepack in the same change: the README's `corepack enable` step and any CI Corepack activation stop working there, so move them to a direct pnpm install or pnpm's native package-manager management.
