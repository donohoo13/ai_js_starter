...
Then the chain takes over: human QA gate first — hand the user steps to see the fix in action and wait for their verdict — then the usual exits (`/stage-for-commit`, or the branch's existing flow). Last, ask what would have prevented this bug: an architectural answer (no good seam, tangled callers, hidden coupling) goes to `/capture-task` with specifics, and a durable debugging gotcha earns one `/codify` nudge. Make these recommendations after the fix is in, not before — you know more now than when you started.
