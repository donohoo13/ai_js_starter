...
- `.mcp.json` — ships `playwright-local` (chromium) and `firefox-devtools`, both version-pinned, with `firefox-devtools` `START_URL` neutral at `about:blank`. → add the project's servers; remove the browser servers if the project has no UI to verify; point `START_URL` at the project's real dev URL once one exists.
