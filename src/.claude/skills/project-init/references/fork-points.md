...
- `scripts/worktree.sh shim` / `scripts/doctor.sh --fix` — the `wtree` shell function is emitted for zsh and bash only, and `--fix` writes to `~/.zshrc` or `~/.bash_profile` (macOS login-shell convention). → fish and other shells get no shim and fall back to `cd "$(scripts/worktree.sh path)"`, which needs nothing; teams standardized on another shell add a branch to `worktree.sh`'s `cmd_shim` and to `doctor.sh`'s `rc_for_shell`. The `wtree` name is probed through the user's login shell at install time (by exit status, never by output) and refused on collision, so it needs no per-project retarget.
- `scripts/worktree.sh shim` pins the emitted function to the absolute path of the `worktree.sh` that emitted it, and the read-only verbs (`wtree`, `wtree cd`, `wtree list`, `wtree path`) run only that pinned copy. → do not "simplify" this to resolve the script from the user's cwd: `wtree` is typed reflexively, including inside repos that were merely cloned, and a cwd-derived `scripts/worktree.sh` is arbitrary code execution on `cd` plus one habit. Mutating verbs (`add`, `remove`) do run the current repo's script, which is what makes per-project retargeting of the install command and env-copy list work at all.
- `scripts/test.sh` — behavioral suite for both scripts; assumes `pnpm` (it commits a lockfile into its throwaway repos), assumes macOS `/bin/bash` 3.2, and asserts the root/`src` twins are byte-identical. → retarget the install command alongside `worktree.sh`'s, wire it into the destination's test runner (`pnpm test` here), and drop the twin assertions: they are a property of this template repo's two-layer shape and meaningless downstream.
