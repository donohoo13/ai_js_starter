...
3. Otherwise, one confirm naming the branch and the worktree target (`${WORKTREE_ROOT:-$HOME/.git-worktrees}/<project>/<branch>`; run `scripts/worktree.sh path` to see where existing ones live). On yes: verify the checkout is on `main` (a checkout parked on another branch seeds the worktree from the wrong tree — surface it and wait), run `scripts/worktree.sh add --no-open <branch>`, then enter via `EnterWorktree` (`path:` the created worktree). This step is the project instruction that authorizes the `EnterWorktree` tool.
...
Declining the confirm is the escape hatch: the user creates or picks a feature branch in the checkout and the build proceeds there — trust whatever non-main branch they choose. The same fallback applies where `scripts/worktree.sh` or the `EnterWorktree` tool is unavailable. Teardown is never this skill's job: post-merge cleanup is `scripts/worktree.sh remove <branch>`, run by the user from the main checkout. When the build runs in a worktree, the human QA script names how the user gets a shell in it: `wtree` if the shim is installed, else `cd "$(scripts/worktree.sh path <branch>)"`.
