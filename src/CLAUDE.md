...
- `scripts/test.sh` — behavioral tests for `scripts/worktree.sh` and `scripts/doctor.sh`, run against a scratch `HOME` and throwaway git repos. Not wired into `prepare` (it creates repos and runs installs); wire it into the manifest's test script.
