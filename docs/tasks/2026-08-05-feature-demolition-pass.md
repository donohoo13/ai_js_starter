...
- `none` and `extend` each carry a one-line why in Design decisions; `replace` carries none. Both no-demolition verdicts cost the same, so neither is the cheap default, and the ruthless option is the cheapest to declare. `implement-task`'s step-1 gate refuses a `none` or `extend` task missing that line, checking only that it is present: an unpaid price is a norm rather than a rule, and weighing whether the reasoning convinces the build session would make it the place the verdict is decided.
