...
- The demolition verdict is decided only during a grilling session, as a precursor to building. `implement-task` never writes the key and has no say in the decision: a missing key is a hard gate that stops the build and routes back to a grilling, on a `scoped` file and a resumed `in-progress` one alike.
...
- The zone arrives from the task file at file level, decided while a human is in the room, never inferred by the agent from a surface name.
...
- The record is session-scoped scratch under the gitignored `.ai/` namespace and is never committed. The obligation to report orphans to the user is met by the manifest landing in the main session's stream during the run.
- Run 2's manifest lands in the dispatching session's stream before the delete commit: visible and interruptible, never a gate that waits.
- Demolition commits its own explicitly-named red state; each subsequent slice shrinks the recorded type-error set and introduces no error outside its own scope; the set is zero before the human QA gate.
...
- [ ] `implement-task` pointed at a task with no `incumbent:` key refuses and routes to a grilling, on both `scoped` and resumed `in-progress` files, and never writes the key itself.
...
- [ ] Whether a subagent declaring `Edit`, `Write`, and `Bash` inherits the session's permission decisions is unverified; run 2 is the suite's first write-capable agent. Verify before wiring; if a subagent cannot delete cleanly, the fallback is the main session executing run 1's plan, which keeps the mandate and loses only the executor's context separation.
...
- [ ] `git show HEAD~1` reaches the whole incumbent on the commit demolition itself creates. The prohibition is stated in the slice loop and is mitigation, not closure; git history has to stay reachable for recovery.
...
- [ ] `git rm` runs under the `Bash` matcher and bypasses `guard-skill-edit`, which matches `Edit|Write` only. In a repo whose payload is `.claude/skills/`, a `replace` task reaches this on the first dispatch.
...
- [ ] Rebase onto `main` and renumber: five files conflict with the merged `v1.5.0` work, and the `CHANGELOG.md` entry plus `package.json` move to `1.6.0` since `1.5.0` is taken. Then two amendments to the landed slices — `implement-task` states it never writes the key, and `none` carries a one-line why alongside `extend`.
- [ ] Rewrite the discipline and the agent to two stages: `implement-task/references/demolition.md` carrying the zone-is-the-only-survival-test rule, the git-recoverability exemption, the blast-radius inventory, the record's filter and artifact-bound phrasing, and the manifest relay; `.claude/agents/demolition.md` split into a read-only planner and a plan-only executor.
- [ ] Rewrite the build wiring: `implement-task` step 3 dispatching run 1, relaying the plan, dispatching run 2, then the named red commit, the connection map recorded in the task file, and the per-slice validate change.
