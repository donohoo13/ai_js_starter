...
- [x] `scripts/test/guard-dev-server.battery.mjs` covers the block and allow cases above by piping crafted tool-call JSON and asserting exit codes and stderr fragments, and passes via `pnpm exec node scripts/test/guard-dev-server.battery.mjs`.
...
- [x] A repo-wide grep for the old licensing language (`this session started (dev servers`, mid-build screenshot mandates, unscoped Playwright verification) returns only history records (`CHANGELOG.md`, `docs/tasks/`, `docs/adr/`).
